...
import {CaffeinesData} from "../../../apis/add_categories";
import {NavLink, useParams} from "react-router-dom";
import CaffeineForm from "../../product_forms/Caffeine";
import CoffeeTypeEdit from "../../product_forms/CoffeeTypeEdit";
import {axiosInstance} from "../../../apis/config";
import {toast} from "react-toastify";
import CaffeineEditForm from "../../product_forms/CaffeineEditForm";


export default function Caffeine() {

    const [show, setShow] = useState(1);
    const coffeeTypes = CaffeinesData();

    const deleteItem = async (e, id)=>{
        e.preventDefault();
        try {
            const res = await axiosInstance.delete(`caffeine/${id}/`);
    ...
    return (
        show === 2? <CaffeineForm submitAdd={submitAdd}/> : show === 3? <CaffeineEditForm submitAdd={submitAdd}/> :
            <div>
                        ...
                        coffeeTypes?.map((item, index) => {
                            return (
                                ...
                                <tr key={item.id}>
                                    ...
                                    <td>
                                        <NavLink
                                            to = {`/admin/caffeine/${item.name}/${item.id}/`}
                                            onClick={()=>{submitAdd(3)}}
