...
import {CreatorsData} from "../../../apis/add_categories";
import {NavLink, useParams} from "react-router-dom";
import CreatorForm from "../../product_forms/Creator";
import CreatorEditForm from "../../product_forms/CreatorEditForm";
...
export default function Creator() {
    ...
    const coffeeTypes = CreatorsData();

    const deleteItem = async (e, id)=>{
        e.preventDefault();
        try {
            const res = await axiosInstance.delete(`creator/${id}/`);
    ...
    return (
        show === 2? <CreatorForm submitAdd={submitAdd}/> : show === 3? <CreatorEditForm submitAdd={submitAdd}/> :
            <div>
                        ...
                        coffeeTypes?.map((item, index) => {
                            return (
                                ...
                                <tr key={item.id}>
                                    ...
                                    <td>
                                        <NavLink
                                            to = {`/admin/creator/${item.name}/${item.id}/`}
                                            onClick={()=>{submitAdd(3)}}
