...
import {RoastingDegrees} from "../../../apis/add_categories";
import {NavLink, useParams} from "react-router-dom";
import RoastingDegreeForm from "../../product_forms/RoastingDegree";
import RoastingDegreeEdit from "../../product_forms/RoastingDegreeEdit";
...
export default function RoastingDegree() {
    ...
    const coffeeTypes = RoastingDegrees();

    const deleteItem = async (e, id)=>{
        e.preventDefault();
        try {
            const res = await axiosInstance.delete(`roastingDegree/${id}/`);
    ...
    return (
        show === 2? <RoastingDegreeForm submitAdd={submitAdd}/> : show === 3? <RoastingDegreeEdit submitAdd={submitAdd}/> :
            <div>
                        ...
                        coffeeTypes?.map((item, index) => {
                            return (
                                ...
                                <tr key={item.id}>
                                    ...
                                    <td>
                                        <NavLink
                                            to = {`/admin/roastingdegree/${item.name}/${item.id}/`}
                                            onClick={()=>{submitAdd(3)}}
